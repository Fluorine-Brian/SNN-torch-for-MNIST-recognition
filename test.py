...
class MNISTInference:
    ...
    def visualize_prediction(self, image_path):
        ...
        try:
            ...
            # 进行预测
            # 注意：这里调用 self.predict 会打印一次结果，可以根据需要保留或移除
            predicted_class, confidence = self.predict(image_path, show_confidence=False)
            if predicted_class is None: # 增加一个健壮性检查
                return None, None

            # --- 代码修改部分 ---
            # 创建一个仅用于可视化的转换，不包含 Normalize
            vis_transform = transforms.Compose([
                transforms.Grayscale(),
                transforms.Resize((28, 28)),
                transforms.ToTensor()
            ])
            processed_for_vis = vis_transform(image)
            # --------------------
            ...
            plt.title(f"Input Figure")
            plt.axis('off')

            # 显示预处理后（未归一化）的图像
            plt.subplot(1, 2, 2)
            # 使用我们新创建的 processed_for_vis
            plt.imshow(processed_for_vis.squeeze().numpy(), cmap='gray')
